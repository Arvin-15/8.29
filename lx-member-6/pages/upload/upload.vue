...
export default {
	...
	methods: {
		...
		blobToDataURL(blob,avatar) {
			var a = new FileReader();
			a.readAsDataURL(blob); //读取文件保存在result中
			a.onload = function(e) {
				var getRes = e.target.result; //读取的结果在result中
				console.log(getRes+""==avatar+"");
				// wx.redirectTo({
				// 	url: '../info/info?avatar='+getRes
				// })
			};
		},
		getCropperImage() {
			...
			this.weCropper.getCropperImage(avatar => {
				if (avatar) {
					//console.log(this.blobToDataURL(this.convertBase64UrlToBlob(avatar),avatar)+""==avatar+"");
					console.log(avatar);
					//  获取到裁剪后的图片
					wx.redirectTo({
					  url: '../info/info?avatar=' + avatar
					})
					//下面是上传到服务器的方法
// 					uni.uploadFile({
// 						url: pathurl,
