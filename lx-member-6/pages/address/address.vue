	...
	<view class="container">
		<view class="address-list">
			<view class="a-address" v-for="(item,index) in addressList" :key="index">
				<view class="left-text" :class="item.isMain==1? 'active':''" @tap="selectTap(item.id)">
					<view class="name-tel">
						{{item.name}} {{item.phone}}
					</view>
					<view class="address-box">
						{{item.province + item.city + item.area + item.street + item.detailedAddress}}
					</view>
				</view>
...
<script>
	import {ADDRESS_LIST,ADDRESS_MODIFY} from '../../api/index.js';
	...
	export default {
		...
		onShow() {
			...
			// let _this = this;
			// console.log(ADDRESS_LIST+"/"+this.userid);
			
			// uni.request({
			// 	url:ADDRESS_LIST+"/"+this.userid,
			// 	header:{
			...
			// 			_this.addressList = res.data.AddressDetails
			// 			uni.setStorageSync("addressList",res.data.AddressDetails);
			// 		}
			// 		uni.hideLoading();
			// 	}
			// })
		...
		methods: {
			async selectTap(id) {
				...
				// 说明点击多次是同一个对象
				if(this.addressList[index].isMain==1)return false
				this.addressList.forEach(item=>item.isMain=0)
				this.addressList[index].isMain=this.addressList[index].isMain==0?1:0
				// 更新默认地址的状态
			...
			async getAddress(){
				...
				if(res.code==SUCCESS)	uni.hideLoading();
				this.addressList=res.AddressDetails;
				uni.setStorageSync("addressList",this.addressList);
			},
			editAddess(id) {
				uni.navigateTo({
