		...
		<view class="flexView">
			<view class="scrollView">
				<view class="head-read">
					<view class="flex">
						<image class="read-img" :src="user.avatar || '../../static/fumou-center-template/header.jpg'" mode="aspectFill" />
						<view class="flex-box">
							<view class="flex-box-text">姓名：{{ user.nickname }}</view>
							<view class="flex-box-text">手机号：{{ user.phone }}</view>
						</view>
				...
				<view class="white-box">
					<view class="tx">
						...
						<view class="tx-grid" v-if="userData.withdrawable>0"
						@click="navTo('/pages/AlipayAccount/AlipayAccount')">
...
<script>
import {QUERY_SCORE} from '../../api/index.js';
...
export default {
	...
	data() {
		return {
			...
			userData: {
				name: "李云龙",
				headimgurl: "/static/fumou-center-template/header.jpg",
				id: "28***06",
				phone: "136***1234",
				withdrawable: 0,
				coming: 0.00,
		...
		uni.request({
			...
			complete: (res) => {
				...
				if(res.data.code == SUCCESS){
					this.userData.withdrawable = res.data.integral;
				}
			}
		})
	},
