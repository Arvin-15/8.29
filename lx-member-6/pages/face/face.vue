<template>
 ...
 -->
		...
		<view class="button-sp-area">
			<button v-if="isFinish" type="primary" plain="true" @click="validAlipay">下一步</button>
		</view>
	</view>
</template>
...
<script>
	...
	export default {
		data() {
			return {
				licenseIDStr: 'lxface-face-android',
				items: [{
						value: 'Eye',
						name: '眨眨眼',
						checked: 'true'
				...
				isLivenessRandom: 1,//动作顺序: 0有序，1随机
				isSound: 1,//1有语音提示  0无语音提示
				frontBack: 0,//0前置摄像头，1后置摄像头
				txtColor: '#3987FD',
				bgColor: '#2F2F33',
				roundColor: '#3987FD',
		...
		mounted() {
			...
			if (uni.getSystemInfoSync().platform == "ios") {
				...
				this.licenseIDStr = "lxface-face-ios";
				...
				this.licenseIDStr = "lxface-face-android";
			}

			//引用插件
			lyBDFaceAuth = uni.requireNativePlugin('longyoung-BDFaceAuth');
			// #endif
			console.log(lyBDFaceAuth);
			this.onScanFace();
		},
		methods: {
			validAlipay(){
